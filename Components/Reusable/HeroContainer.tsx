...
const Hero: React.FC<HeroProps> = ({
  ...
  useEffect(() => {
          ...
          if (!isMobile) {
            const clipPath =
              progress > 0.3
                ...
                : `polygon(
                    0% 0%, ${25 - progress * 25}% ${2 - progress * 2}%, 50% ${
                    4 - progress * 4
                  }%, ${75 + progress * 25}% ${2 - progress * 2}%, 100% 0%,
                    100% 100%, ${75 + progress * 25}% ${
                    98 + progress * 2
                  }%, 50% ${96 + progress * 4}%, ${25 - progress * 25}% ${
                    98 + progress * 2
                  }%, 0% 100%
                  )`;

            const borderRadius =
              progress >= 0.8 ? "0px" : `${40 - progress * 50}px`;
            ...
            const borderRadius =
              progress >= 0.8 ? "0px" : `${20 - progress * 20}px`;
            gsap.set(heroElement, {
              borderRadius,
            });
          }
    ...
    tl.fromTo(
      containerElement,
      { padding: "16px" },
      { padding: "0px", duration: 1, ease: "power2.out" },
      0
    );
          ...
          <Container className="w-full">
            <div className="text-white flex flex-col md:flex-row justify-between items-start md:items-end gap-4 md:gap-0">
              <h1
                className={`${
                  isLeft ? "block" : "hidden"
                } text-4xl md:text-7xl leading-tight md:leading-20 font-bodoni font-medium text-left ${
                  content.fontClassName || ""
                }`}
              >
              ...
              <div
                className={`${
                  isRight ? "flex" : "hidden"
                } flex-col items-start w-full md:w-auto`}
              >
