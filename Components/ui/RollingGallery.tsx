...
const RollingGallery: React.FC<RollingGalleryProps> = ({
    ...
    // FIXED: Adjusted calculations to show 5 cards with proper spacing
    const cylinderWidth: number = isScreenSizeSm ? 1800 : 2400; // Reduced cylinder width
    const faceCount: number = galleryImages.length;
    const faceWidth: number = (cylinderWidth / faceCount) * 1.1; // Increased face width ratio
    const radius: number = cylinderWidth / (1.6 * Math.PI); // Changed from 2.5 to 2 for tighter radius
                    ...
                    {galleryImages.map((item, i) => (
                        ...
                        >
                            ...
                            <div className="relative h-[300px] w-[280px] sm:h-[280px] sm:w-[260px] rounded-[15px] overflow-hidden transition-transform duration-300 ease-out group-hover:scale-105">
                                ...
                                {/* Bottom blur effect */}
                                <div className="absolute bottom-0 left-0 right-0 h-20 bg-gradient-to-t from-black/80 via-black/40 to-transparent backdrop-blur-[3px]" />
                            </div>
                        </div>
                    ))}
                </motion.div>
